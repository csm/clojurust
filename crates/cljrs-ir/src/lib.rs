...
pub enum Inst {
    ...
    /// `RegionAlloc` instructions in the callee body, but the actual
    /// allocation target comes from the thread-local region stack — so the
    /// runtime can treat this as a placeholder bind to nil.
    RegionParam(VarId),

    /// Direct call to a region-parameterised callee variant.
    ///
    /// `(dst, target_name, args)` — semantically identical to `CallDirect`,
    /// but signals that the call site has wrapped itself in
    /// `RegionStart`/`RegionEnd` so that the callee's `RegionAlloc`
    /// instructions can allocate into the caller's region.  Generated by the
    /// stage-4 cross-function region-promotion pass.
...
impl Inst {
    ...
    pub fn dst(&self) -> Option<VarId> {
        match self {
            ...
            | Inst::RegionParam(v)
            | Inst::CallWithRegion(v, _, _) => Some(*v),
    ...
    pub fn uses(&self) -> Vec<VarId> {
        match self {
            ...
            Inst::CallWithRegion(_, _, args) => args.clone(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Inst::CallWithRegion(dst, name, args) => {
                write!(f, "{dst} = call_with_region {name} {args:?}")
            }
