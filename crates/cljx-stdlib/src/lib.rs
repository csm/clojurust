...
//! ## Entry points
//!
//! - [`standard_env()`] — full environment for the `cljx` binary
...
///
/// Prefer this over `cljx_eval::standard_env()` in the `cljx` binary so that
/// stdlib namespaces are loaded lazily (only on first `require`) instead of
/// eagerly at startup.
pub fn standard_env() -> Arc<GlobalEnv> {
    let globals = cljx_eval::standard_env_minimal();
    register(&globals);
    globals
}
...
mod tests {
    use super::*;
    use cljx_eval::{Env, EvalResult, eval};
