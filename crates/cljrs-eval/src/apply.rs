...
pub fn call_cljrs_fn(f: &CljxFn, args: Vec<Value>, caller_env: &mut Env) -> EvalResult {
    ...
    // Try IR path if compiler is ready and this isn't a macro.
    if !f.is_macro {
    ...
    call_cljrs_fn_tree_walk(f, arity, args, caller_env)
}
...
fn execute_ir(
    ...
    env.push_frame();
    bind_fn_params(arity, args, &mut env)?;
