...
| `src/jit_compiler.rs` | `compile_jit(func_name, ir_func, specs)` — builds `JITModule`, registers rt_abi symbols, recursively declares + compiles closure subfunctions into the same module (mirroring AOT; subfunctions always compile generic), calls shared codegen (`compile_function_with_specs` for the top-level function); returns `CompiledFn { fn_ptr, module, code_size }`. `compile_jit_poll(name, ir_func)` is the async variant — declares the state-machine poll ABI (`declare_poll_function`) for an `is_async_poll_fn` IR function |
| `src/async_jit.rs` | Async-JIT activation (Phase H): `compile_async_arity` (installed by `init` as the `cljrs_env` async-compile hook) lowers a called `^:async` arity (`lower_arity` with `is_async`, **no region pass** — a region scope must not span a suspend), runs `lower_async`, JIT-compiles the poll fn via `compile_jit_poll`, keeps the module alive (`KEEPALIVE`), and registers it in `cljrs_async`'s poll-fn registry under the arity's `ir_arity_id`. Capturing closures and unsupported constructs bail to the `eval_async` tree-walker |
