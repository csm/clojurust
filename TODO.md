...
- [~] Closures with captured bindings: complete capture lowering + closure-alloc codegen — **blocked on pre-existing JIT codegen bugs.** Inner closures lower fine (Tier-1 runs them correctly), but the JIT compiles a single arity at a time and does not declare its closure subfunctions, so `AllocClosure` codegen cannot resolve them. Compiling the subfunctions (as AOT does) exposes a separate codegen bug: a closure built by `rt_make_fn` inside JIT code and then invoked via `rt_call` returns `nil` (a closure *passed in* from the interpreter works). Until that is fixed, enabling closure JIT would regress correctness, so codegen instead **declines closures gracefully** (`lookup_user_func` returns a clean error instead of panicking) and they fall back to Tier-1. The worker is now panic-resilient (`catch_unwind`) so one un-compilable function can't disable the JIT for the rest of the session.
...
- [~] Promote special-cased ops to first-class `KnownFn`/IR instructions — `apply`, `atom`, `reset!`, `swap!`, `deref` are already `KnownFn`s with rt_abi bridges (mapped in `cljrs-ir/src/lower/known.rs`); `volatile!`/`vswap!`/`vreset!` remain. Deferred alongside the closure codegen work above: adding more rt_abi-call codegen surface to a tier with open `rt_call`/HOF correctness bugs is not yet net-positive.

  **Discovered pre-existing JIT-native correctness bugs** (independent of this phase — they affect functions that were already JIT-eligible): higher-order calls taking a function value (`reduce`/`map`/`filter` with `+`, `inc`, `even?`, or a lambda) and in-place calls of a JIT-constructed closure currently return `nil` under JIT-native dispatch. Tier-1 and the tree-walker are correct. These need dedicated codegen/runtime debugging before the JIT tier can be trusted for closures and HOFs.
