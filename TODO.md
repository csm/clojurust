...
- [ ] `wasm-encoder` emitter — unboxed **parameter** ABI aligned with `function_signature` (unboxed params + boxed-entry trampoline for `rt_call`/cross-function callers + entry guards); intermediates are already unboxed
...
- [ ] Bundling: link the emitted module with the runtime built for `wasm32-unknown-unknown` (satisfy the `"rt"` imports; finalize `abi::RODATA_BASE`/`abi::FUNC_TABLE_BASE` against its layout) and bundle cross-namespace dependencies (not just the entry ns)
