...
- [ ] `wasm-encoder` emitter (remaining `Inst`s): `Call`/`CallDirect`/`CallWithRegion`, `LoadGlobal`/`LoadVar`/`DefVar`, string/keyword/symbol constants (data segment), unboxed `Long`/`Double` specialization aligned with `function_signature`
