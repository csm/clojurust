...
- [ ] `wasm-encoder` emitter — closures + function table: `AllocClosure` via a `ref.func` element segment (wasm function pointers are table indices) + `rt_make_fn*`; the `rt_call_ic` inline cache (writable per-call-site IC slot)
- [ ] `wasm-encoder` emitter (remaining `Inst`s): `LoadGlobal`/`LoadVar`/`DefVar`, string/keyword/symbol constants (data segment), unboxed `Long`/`Double` specialization aligned with `function_signature`
